...
SPECIAL(hometown_room)
{
  ...
  if (!*argument) {
    sprintf(buf,"&wThere are following hometowns available to you in &CVision&cMUD&w:\r\n");  
...
SPECIAL(bank)
{
  ...
  if (CMD_IS("balance")) {
    ...
    if ((vict = get_char_vis(ch, buf2))) {
      ...
      sprintf(buf, "&r-&m=&M#&m=&r- &WFirst National Bank of VisionMUD &r-&m=&M#&m=&r-&w\r\n\r\n");
  ...
  if(i <= 10) {  /* slow target for a few rounds */
    ...
    switch(k) {
      ...
      case 4:
        ...
        break;
      case 5:
        act("The heavens release a torrent of rain, blurring your visibility, 
and making movement difficult!"          , FALSE, ch, 0 , tch, TO_CHAR);
        break;
      case 6:
        k=number(1,3);
        switch(k) {
      ...
      case 7:
        ...
        break;
      case 8:
        act("A stream of 600 large butterflies pours forth from the $p -
Blinding everyone in the room!",FALSE,ch,wand,tch,TO_CHAR);
        break;
      case 9:
      ...
      case 10:
        ...
        break;
      case 11:
             act("The plants and grasses in the area surge forth in
unparalled
growth!",
      ...
      case 16:
        ...
        break;
      case 17:
        act("A stream of gemstones is expelled from the $p, striking all
those in its path!",
                 FALSE, ch, wand, tch, TO_CHAR);
        break;
      case 18:
        act("Shimmering colors dance and play in the area - blinding all
in the area!", FALSE,
                ch, 0, tch, TO_CHAR);
        break;
      case 19:
...
and making movement difficult!"
   ...
   /* so, I guess, PUFF must exist. (vn 1) */
    ...
    if(k==1) { /* rhino */
      ...
      mob->player.description=str_dup("The beast looks quite
dangerous....");
      GET_LEVEL(mob)=20;
      GET_MAX_HIT(mob)=350;
      GET_HIT(mob)=350;
      ...
      mob->player.description=str_dup("This raging bull elephant looks
really pissed off!");
      GET_LEVEL(mob)=15;
      GET_MAX_HIT(mob)=650;
      GET_HIT(mob)=650;
      ...
      mob->player.description=str_dup("The mouse looks more interested in
running than fighting!");
    ...
    act("A stream of 600 large butterflies pours forth from the $p -
blinding everyone in the room!",FALSE,ch,wand,tch,TO_ROOM);
    for (tch = world[ch->in_room].people; tch; tch = next_tch) {
      next_tch = tch->next_in_room;
      call_magic(ch, tch, 0, 0, SPELL_BLINDNESS, 3, CAST_SPELL);
    }
    ...
    act("All the vegetation in the area suddenly doubles - no quadruples
in size!", FALSE,ch,0,tch,TO_ROOM);
    ...
    act("$N is engulfed in a nasty fireball!", FALSE, ch, 0, tch,
TO_ROOM);
    act("You are engulfed in a nasty fireball!", FALSE, ch, 0 , tch,
TO_VICT);
    ...
    act("How odd - $N starts to sprout leaves and thorny bristles!",
FALSE, ch, 0, tch, TO_ROOM);
    ...
    act("A spray of well cut gems sprays forth from the $p, striking all
in the room!",
         FALSE, ch, 0 , tch, TO_ROOM);
    for (tch = world[ch->in_room].people; tch; tch = next_tch) {
      next_tch = tch->next_in_room;
    ...
    act("An amazing field of shimmering colors plays over the entire
area!", FALSE,
     ch, 0, tch, TO_ROOM);
    ...
    for (d = descriptor_list; d; d = d->next) {
      if(!IS_NPC(d->character) && (world[d->character->in_room].zone ==
        ...
        send_to_char("A bright flash of color blinds
you!\r\n",d->character);
