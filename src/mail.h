...
/* Magic string used to separate ascii-saved object from rest of mail */
#define MAGIC_STRING	"\06OBJ\n\06"

#define MAX_MAIL_TEXT_SIZE	4096

/* Maximum size of mail in bytes (arbitrary)	*/
#define MAX_MAIL_SIZE	(MAX_MAIL_TEXT_SIZE+2048)

/* Maximum number of objects stored in mail */
#define MAX_MAIL_OBJECTS	6
...
char	*read_delete(long recipient, struct obj_data **obj);


#define HEADER_BLOCK  -1
#define LAST_BLOCK    -2
#define DELETED_BLOCK -3
#define OBJECT_BLOCK  -4
#define OBJTERM_BLOCK -5
...
struct object_block_type_d {
   long block_type;
   struct char_file_u object;
};

typedef struct header_block_type_d header_block_type;
typedef struct data_block_type_d data_block_type;
typedef struct object_block_type_d object_block_type;
...
struct mail_index_type_d {
   ...
   byte notified;			/* Whether recipient notified	*/
