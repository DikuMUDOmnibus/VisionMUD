  ...
  if (IS_NPC(i)) {
    ...
    if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_SHOWCLAN) && PLAYERCLAN(i) != 0 &&
      ...
      && CLANRANK(i) > CLAN_APPLY)
        sprintf(buf1,"&w (%s)",CLANNAME(clan_index[PLAYERCLANNUM(i)]));
      else
        strcpy(buf1,"");
...
ACMD(do_exits)
{
  ...
  *buf = '\0';
  ...
  if (AFF_FLAGGED(ch, AFF_BLIND)) {
    send_to_char("Your blind eyes see less than you thought they
would.\r\n", ch);
    return;
  }
  for (door = 0; door < NUM_OF_DIRS; door++) {
...
ACMD(do_score)
{
  ...
  if (IS_NPC(ch) || GET_LEVEL(ch) >= 18)
    sprintf(buf + strlen(buf), "Hitroll: [&g%d&w], Damroll: [&g%d&w]\r\n",
  ...
  if (!IS_NPC(ch)) {
    ...
    sprintf(buf + strlen(buf), "This ranks you as &C%s &W%s&w (level &M%d&w).\r\n",
...
ACMD(do_who)
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (GET_LEVEL(wch) >= LVL_IMMORT) {
      ...
      sprintf(Imm_buf, "%s%s&w[%s&w]&W %s %s&w%s", Imm_buf, CCYEL(ch, C_SPR), 
      ...
      if ((PLAYERCLAN(wch) != 0) && (CLANRANK(wch) > CLAN_APPLY)) {
        ...
        strcpy(buf,"");
      sprintf(Mort_buf, "%s&w[&M%2d %s&w]&W %s %s&w%s", Mort_buf, GET_LEVEL(wch),
      	      CLASS_ABBR(wch), GET_NAME(wch), GET_TITLE(wch), buf);
      Mortals++;
    }
  ...
  if ((Wizards + Mortals) == 0)
    strcpy(buf, "&RNo wizards or mortals are currently visible to you.\r\n");
  if (Wizards)
    sprintf(buf, "There %s &C%d visible immortal%s&w%s", (Wizards == 1 ? "is" : "are"), Wizards, (Wizards == 1 ? "" : "s"), (Mortals ? " and there" : "."));
  if (Mortals)
    sprintf(buf, "%s %s &G%d visible mortal%s&w.", (Wizards ? buf : "There"), (Mortals == 1 ? "is" : "are"), Mortals, (Mortals == 1 ? "" : "s"));
  strcat(buf, "\r\n");
