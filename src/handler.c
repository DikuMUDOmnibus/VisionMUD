...
int isname_list(char *what, char *list)
{
  strcpy(buf1, what);
  while (buf1 && *buf1) {
    half_chop(buf1, buf, buf1);
    if (!isname(buf, list)) return 0;
  }
  return 1;
}

void affect_modify(struct char_data * ch, byte loc, sbyte mod, long_bitv bitv,
		        bool add)
{
  int maxabil, i;
//  if (!IS_NPC(ch)) dump_char(ch);
  if (add) {
    for (i=0; i<LONG_BITV_WIDTH; i++)
      SET_BIT(AFF_FLAGS_BITV(ch)[i], bitv[i]);
  } else {
    for (i=0; i<LONG_BITV_WIDTH; i++)
      REMOVE_BIT(AFF_FLAGS_BITV(ch)[i], bitv[i]);
    mod = -mod;
  }
...
void affect_remove(struct char_data * ch, struct affected_type * af)
{
  ...
  /* return corpse host people to their proper bodies */
    if ((af->type = SPELL_CORPSE_HOST) && ch->desc && ch->desc->original)
     {
...
void obj_to_char(struct obj_data * object, struct char_data * ch)
{
  if (object && ch) {
    ...
    if (IS_OBJ_STAT(object, ITEM_AUTOENGRAVE) 
      && !IS_OBJ_STAT(object, ITEM_ENGRAVED) && !IS_NPC(ch)) {
      GET_OBJ_OWNER_ID(object) = GET_IDNUM(ch);
...
void equip_char(struct char_data * ch, struct obj_data * obj, int pos)
{
  ...
  if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch)) ||
      ...
      invalid_class(ch, obj) || invalid_level(ch, obj)) {
...
void obj_to_room(struct obj_data * object, room_rnum room)
{
  if (!object || room < 0 || room > top_of_world)
    ...
    SECT(room) == SECT_WATER_NOSWIM) && !OBJ2_FLAGGED(object, ITEM2_NOSINK)) {
...
void extract_char(struct char_data * ch)
{
  ...
  if (!IS_NPC(ch)) {
    ...
    Crash_delete_crashfile_objects(ch);
  } else {
