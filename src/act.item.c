...
int can_take_obj(struct char_data * ch, struct obj_data * obj)
{
  if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
    ...
    return 0;
  } else if (IS_SET(GET_OBJ_EXTRA(obj), ITEM_ANTI_MORT) && GET_LEVEL(ch) < LVL_IMMORT) {
    act("$p: you don't feel to be godly enough to touch it!", FALSE, ch, obj, 0, TO_CHAR);
    return 0;
  } else if (GET_OBJ_BOTTOM_LEV(obj) > GET_LEVEL(ch)) {
    act("$p: you don't feel you're experienced enough to handle it!", FALSE, ch, obj, 0, TO_CHAR);
    return 0;
  } else if (GET_OBJ_TOP_LEV(obj) < GET_LEVEL(ch)) {
    act("$p: you wouldn't touch such a newbie item!", FALSE, ch, obj, 0, TO_CHAR);
    return 0;
  }
  return 1;
}
...
void perform_wear(struct char_data * ch, struct obj_data * obj, int where)
{
  ...
  const char *already_wearing[] = {
    ...
    "You're already something put on four face.\r\n"
  };
  ...
  if (IS_OBJ_STAT(obj, ITEM_ENGRAVED) && 
    (IS_NPC(ch) || GET_OBJ_OWNER_ID(obj) != GET_IDNUM(ch))) {
    send_to_char("But it does not belong to you. It's engraved to someone else!\r\n", ch);
    return;  
  }
...
void perform_remove(struct char_data * ch, int pos)
{
  ...
  else if (IS_OBJ_STAT(obj, ITEM_NODROP) && GET_LEVEL(ch) < LVL_IMMORT)
...
char* msgs[][2] = {
	...
	{"You can't dig up air!\r\n",
	 NULL },
	{"You can't dig in the mud! Try drop that in.\r\n",
	 NULL },
	/* always keep this as the last message */
	{ "If you see this please tell a god.\r\n", NULL }
};

const int dig_remains[] = {60, 61, 62, 63, 64, 65};
const int time_remains[] = {2, 2, 3, 3, 3, 1};
const int dig_hole[] = {70, 71, 71, 71, 71, 71};
const int time_hole[] = {2, 2, 5, 5, 5, 4};

void create_remains(struct char_data * ch)
{
  struct obj_data *obj = NULL;
  if (SECT(ch->in_room) > SECT_MOUNTAIN) return;
  obj = read_object(dig_remains[SECT(ch->in_room)], VIRTUAL);
  if (!obj) {
    log("SYSERR: cannot create remains after burying.");
    return;
  }
  if (GET_CLASS(ch) != CLASS_RANGER)
    GET_OBJ_TIMER(obj) = time_remains[SECT(ch->in_room)];
  obj_to_room(obj, ch->in_room);

}

void create_hole(struct char_data * ch)
{
  struct obj_data *obj = NULL;
  if (SECT(ch->in_room) > SECT_MOUNTAIN) return;
  obj = read_object(dig_hole[SECT(ch->in_room)], VIRTUAL);
  if (!obj) {
    log("SYSERR: cannot create hole.");
    return;
  }
/*  if (GET_CLASS(ch) != CLASS_RANGER) */
  GET_OBJ_TIMER(obj) = time_hole[SECT(ch->in_room)];
  obj_to_room(obj, ch->in_room);
}
...
ACMD(do_bury)
{
	...
	if (OBJ_FLAGGED(obj, ITEM_NODROP)) {
	  sprintf(buf, "You want to bury %s %s but you can't. Damn! It must be cursed.\r\n", 
	     AN(obj->name), obj->name);
	  send_to_char(buf, ch);
	  return;
	 
	}
	if (GET_MOVE(ch) < 20) {
		send_to_char("You are too tired to bury that now.\r\n", ch);
		return;
	}
	/*
	...
	*/
	if((SECT(IN_ROOM(ch)) == SECT_WATER_SWIM) ||
		...
		if(msgs[SECT(IN_ROOM(ch))][0] != NULL)
			send_to_char(msgs[SECT(IN_ROOM(ch))][0], ch);
		if(msgs[SECT(IN_ROOM(ch))][1] != NULL)
			act(msgs[SECT(IN_ROOM(ch))][1], TRUE, ch, NULL, NULL,TO_ROOM);
		return;
	}
	...
	act("You bury $a $o here.", TRUE, ch, obj, NULL, TO_CHAR);
	act("$n buries $a $o here.", TRUE, ch, obj, NULL, TO_ROOM);
...
ACMD(do_dig)
{
	...
	*/
	if (GET_MOVE(ch) < 20) {
		send_to_char("You are too tired to dig now.\r\n", ch);
		return;
	}
	if((SECT(IN_ROOM(ch)) == SECT_WATER_SWIM) ||
		(SECT(IN_ROOM(ch)) == SECT_WATER_NOSWIM) ||
	...
	*/
	...
	while(obj != NULL) {
		...
		{
			if (GET_MOVE(ch) < 20) {
		          send_to_char("You get VERY tired by digging. You need rest!\r\n", ch);
		          return;
		        }
			...
			if(CAN_SEE_OBJ(ch, obj)) {
				...
				act("You found $a $o buried here.", TRUE, ch, obj, NULL, TO_CHAR);
				act("$n has found $a $o buried here.", TRUE, ch, obj, NULL,TO_ROOM);
            			obj_from_room(obj);
				obj_to_char(obj, ch);
				GET_MOVE(ch) -= 20;  
			}
