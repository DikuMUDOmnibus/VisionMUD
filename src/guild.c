...
void list_skills(struct char_data * ch)
{
  ...
  if (IS_NPC(ch)) {
    send_to_char("You are unable to learn any skill.\r\n", ch);
    return;
  }
  if (!GET_PRACTICES(ch))
    strcpy(buf2, "&WYou have &Rno&W practice sessions remaining.&w\r\n");
  ...
  strcat(buf2, "&MYou know of the following spells:\r\n");
  ...
  for (sortpos = 1; sortpos < MAX_SKILLS; sortpos++) {
    ...
    if (!IS_SKILL(i)) {
      ...
      if (GET_LEVEL(ch) >= spell_info[i].min_level[(int) GET_CLASS(ch)]) {
        sprintf(buf2 + strlen(buf2), "&c%-20s &g%-15s &w[&W%3d&w]\r\n", spells[i], how_good(GET_SKILL(ch, i)),
        mag_manacost(ch, i));
        found++;
      }
    } 
  ...
  strcat(buf2, "\r\n");
  found = 0;
  strcat(buf2,"&MYou know of the following skills:&w\r\n");
  for (sortpos = 1; sortpos < MAX_SKILLS; sortpos++) {
    i = spell_sort_info[sortpos];
    if (IS_SKILL(i)) {
      if (strlen(buf2) >= MAX_STRING_LENGTH - 80) {
      ...
      if (GET_LEVEL(ch) >= spell_info[i].min_level[(int) GET_CLASS(ch)]) {
        sprintf(buf2 + strlen(buf2), "&c%-20s &g%-15s&w\r\n", spells[i], how_good(GET_SKILL(ch, i)));
        found++;
      }
    }
  }
