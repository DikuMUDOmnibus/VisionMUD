...
{ struct zone_data *zone;
  ...
  zone->creator = str_dup(zone_table[OLC_ZNUM(d)].creator);
  zone->master_mob = zone_table[OLC_ZNUM(d)].master_mob;
  zone->owner = zone_table[OLC_ZNUM(d)].owner;
  zone->unloaded = zone_table[OLC_ZNUM(d)].unloaded;
  zone->locked = zone_table[OLC_ZNUM(d)].locked;
  zone->zon_flags = zone_table[OLC_ZNUM(d)].zon_flags;
  ...
  for(i = 0; i <= top_of_zone_table + 1; i++)  
    if(!found)
      ...
      { found = 1;
        ...
        new_table[i].creator = str_dup("No subscription.");
        new_table[i].zon_flags = 0;
        new_table[i].master_mob = 0;
        new_table[i].owner = 0;
        new_table[i].unloaded = FALSE;
        new_table[i].locked = FALSE;
        new_table[i].builders = str_dup(NONESTR);
...
{ int subcmd = 0, cmd_room = -2, room_num;
  ...
  { free(zone_table[OLC_ZNUM(d)].name);
    free(zone_table[OLC_ZNUM(d)].creator);
    ...
    zone_table[OLC_ZNUM(d)].creator 	= str_dup(OLC_ZONE(d)->creator);
    zone_table[OLC_ZNUM(d)].master_mob 	= OLC_ZONE(d)->master_mob;
    zone_table[OLC_ZNUM(d)].zon_flags 	= OLC_ZONE(d)->zon_flags;
    zone_table[OLC_ZNUM(d)].owner 	= OLC_ZONE(d)->owner;
    zone_table[OLC_ZNUM(d)].unloaded 	= OLC_ZONE(d)->unloaded;
...
{ int subcmd, arg1 = -1, arg2 = -1, arg3 = -1;
  ...
  sprintf(buf, 
	...
	zone_table[OLC_ZNUM(d)].creator ? zone_table[OLC_ZNUM(d)].creator : "no subscription",
	zone_table[OLC_ZNUM(d)].master_mob,
	zone_table[OLC_ZNUM(d)].zon_flags,
  ...
  fprintf(zfile, "S\n%s\n$\n", 
  (zone_table[OLC_ZNUM(d)].builders && *zone_table[OLC_ZNUM(d)].builders)
               ? zone_table[OLC_ZNUM(d)].builders : NONESTR);
...
{ int subcmd = 0, room, counter = 0;
  ...
  sprintbit(ZON_FLAGS(OLC_ZONE(d)), zon_bits, buf1);
  strcpy(buf2, "Error!");
  if (OLC_ZONE(d)->master_mob != 0) {
    if (real_mobile(OLC_ZONE(d)->master_mob) < 0) {
      sprintf(buf2,"%d (Not existing!)", OLC_ZONE(d)->master_mob);
    } else {
      sprintf(buf2,"%d (%s)", OLC_ZONE(d)->master_mob, 
      mob_proto[real_mobile(OLC_ZONE(d)->master_mob)].player.short_descr);
    }
  }
  ...
  sprintf(buf,
	...
	"Room number: %s%d%s		Room zone: %s%d\r\n"
        ...
        "%sC%s) Creator     : &m%s\r\n"
        "%sB%s) Builders    : &m%s\r\n"
        "%sL%s) Lifespan    : &m%3d minutes		%sT%s) Top of zone : &m%d\r\n"
        "%sR%s) Reset Mode  : &m%s\r\n"
        "%sM%s) Master Mob  : &m%s\r\n"
        "%sF%s) Zone Flags  : &c%s\r\n"
	...
	cyn, zone_table[OLC_ZNUM(d)].number,
        grn, nrm, OLC_ZONE(d)->name ? OLC_ZONE(d)->name : NONESTR,
        grn, nrm, OLC_ZONE(d)->creator,
        grn, nrm, OLC_ZONE(d)->builders ? OLC_ZONE(d)->builders : NONESTR,
        grn, nrm, OLC_ZONE(d)->lifespan,
        grn, nrm, OLC_ZONE(d)->top,
        grn, nrm, OLC_ZONE(d)->reset_mode ? 
          ((OLC_ZONE(d)->reset_mode == 1) ?
          "Reset when no players are in zone." : 
          "Normal reset.") :
          "Never reset",
        
        grn, nrm, OLC_ZONE(d)->master_mob ? buf2 : "Not set.",
        grn, nrm, buf1
  );
...
{ get_char_cols(d->character);
  sprintf(buf, "[H[J"
    ...
    "%sR%s) Remove an object from the room  %sQ%s) Quit to main menu\r\n"
    "What sort of command will this be? (? to help) : ",
...
/*-------------------------------------------------------------------*/
/*. Display zon-flags menu .*/

void zedit_disp_zon_flags(struct descriptor_data *d)
{ 
  
  get_char_cols(d->character);
  send_to_char("[H[J", d->character);
  menubit(&ZON_FLAGS(OLC_ZONE(d)), zon_bits, buf, 
    CHECK_MASK_PERM(d, zon_flags_mask));
  send_to_char(buf, d->character);
  sprintbit(ZON_FLAGS(OLC_ZONE(d)), zon_bits, buf1);
  sprintf(buf, "\r\n"
	"Current flags   : %s%s%s\r\n"
...
{ int pos, i = 0;
  ...
  if (arg && *arg) {
    ...
    if (str_cmp(buf, "?") == 0) {
      ...
      OLC_MODE(d) = ZEDIT_HELP;
      if (*buf1 != '\0')
        do_help(d->character, buf1, 0, 0);
      else
      switch (OLC_LAST(d)) {  /* Here define help invokations for input modes */
        case ZEDIT_MAIN_MENU:
          do_help(d->character, "ZEDIT MAIN MENU", 0, 0);      
          break;
        case ZEDIT_ZON_FLAGS:
          do_help(d->character, "ZONE FLAGS", 0, 0); 
          break;
        case ZEDIT_COMMAND_TYPE:
          do_help(d->character, "ZEDIT COMMANDS", 0, 0); 
          break;
        default:  /* Do nothing */
    ...
    if (OLC_MODE(d) == ZEDIT_HELP) return;
  }
...
/*-------------------------------------------------------------------*/
  case ZEDIT_MAIN_MENU:
    ...
    { case 'q':
      case 'Q':
        if(!OLC_READONLY(d) && (OLC_ZONE(d)->age || OLC_ZONE(d)->number))
      ...
      case 'c':
      case 'C':
        send_to_char("Enter creator's name : ", d->character);
        OLC_MODE(d) = ZEDIT_CREATOR;
        break;
      case 'b':
      case 'B':
        if (GET_LEVEL(d->character) < LVL_CHBUILD) {
          send_to_char("Only Chief Builders and above can modify the builder list.\r\n", d->character);
          zedit_disp_menu(d);
        } else { 
          /*
      ...
      case 'm':
      case 'M':
        send_to_char("Enter VNUM of new zone master (0 to disable): ", d->character);
        OLC_MODE(d) = ZEDIT_MASTER_MOB;
        break;
      case 'f':
      case 'F':
        OLC_MODE(d) = ZEDIT_ZON_FLAGS;
        zedit_disp_zon_flags(d);
        return;
      default:
        zedit_disp_menu(d);
        break;
...
/*-------------------------------------------------------------------*/
  case ZEDIT_CREATOR:
    /*. Add creator's name and return to main menu .*/
    free(OLC_ZONE(d)->creator);
    OLC_ZONE(d)->creator = str_dup(arg);
    OLC_ZONE(d)->number = 1;
    zedit_disp_menu(d);
    break;
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ZONE_LIFE:
    ...
    break; 
/*-------------------------------------------------------------------*/
  case ZEDIT_MASTER_MOB:
    /*. Change master mobile of the zone .*/
    pos = atoi(arg);
    if (real_mobile(pos) < 0 && pos != 0) 
      send_to_char("Such mob does not exist try again (0 to disable) : ", d->character);
    else
      { OLC_ZONE(d)->master_mob = pos;
        OLC_ZONE(d)->number = 1;
        zedit_disp_menu(d);
      }
    break; 
...
case ZEDIT_ZON_FLAGS:
    ...
    updatebit(&ZON_FLAGS(OLC_ZONE(d)), NUM_ZON_FLAGS, i, 
      CHECK_MASK_PERM(d, zon_flags_mask));
    OLC_ZONE(d)->number = 1;
    zedit_disp_zon_flags(d);
    return;
  ...
  case ZEDIT_HELP:
    ...
    switch (OLC_MODE(d)) {
      ...
      case ZEDIT_COMMAND_TYPE:
        ...
        break;
      default:
        log("SYSERR: Help topic with undefined return in zedit (mode = %d)", OLC_MODE(d));
        break;
    }
    return;
/*-------------------------------------------------------------------*/
