 ...
 int     mprog_seval		(const char *lhs, const char *opr, const char *rhs);
 ...
 void	mprog_process_cmnd	(char* cmnd, struct char_data* mob, 
 				       struct char_data* actor, struct obj_data* obj,
 				       void* vo, struct char_data* rndm);
 char *	mprog_driver		(char* com_list, struct char_data* mob,
 				       struct char_data* actor, struct obj_data* obj,
 				       void* vo);
 void mprog_new_program(char *comlist, struct char_data *mob, struct char_data *actor,
 ...
 int mprog_feval(char *val, const char *list[], long bitv[], int max)
 {
   if (!isdigit(*val)) 
     return (get_bitv_from_string(val, list, bitv, max));
   else    
     return (bitv[0] & atoi(val)); 
 }
...
//   int       numarg;
   ...
   if (*point == '\0') 
     {
       bug ("Mob: %d null ifchck", (int)mob_index[mob->nr].vnum); 
       return -1;
     }   
   /* skip leading spaces */
   while (*point == ' ')
   ...
   while (*point != '(') 
     if (*point == '\0') 
       {
 	bug ("Mob: %d ifchck syntax error", mob_index[mob->nr].vnum); 
 	return -1;
       }   
     else
       if (*point == ' ')
   ...
   *argpt = '\0';
   ...
   if (!str_cmp(buf, "isaffected"))
     {
       switch (arg[1])  /* arg should be "$*" so just get the letter */
 	{
 	case 'i': return mprog_feval(val, affected_bits,
 	                   mob->char_specials.saved.affected_by, NUM_AFF_FLAGS);
 	case 'n': if (actor)
 	             return mprog_feval(val, affected_bits,
 	                   actor->char_specials.saved.affected_by, NUM_AFF_FLAGS);
 	          else return -1;
 	case 't': if (vict)
 	             return mprog_feval(val, affected_bits,
 	                   vict->char_specials.saved.affected_by, NUM_AFF_FLAGS);
 	          else return -1;
 	case 'r': if (rndm)
 	             return mprog_feval(val, affected_bits,
 	                   rndm->char_specials.saved.affected_by, NUM_AFF_FLAGS);
 	          else return -1;
 	default:
   ...
   if (!str_cmp(buf, "clannum"))
 	...
 	case 'n': if (actor)
 	          {
 		    lhsvl = IS_NPC(actor) ? 0 : PLAYERCLAN(actor);
 		    rhsvl = atoi(val);
 		    return mprog_veval(lhsvl, opr, rhsvl);
 		  }
 	          else
 	...
 	case 't': if (vict)
 	          {
 		    lhsvl = IS_NPC(vict) ? 0 : PLAYERCLAN(vict);
 		    rhsvl = atoi(val);
 		    return mprog_veval(lhsvl, opr, rhsvl);
 		  }
 	          else
 	...
 	case 'r': if (rndm)
 	          {
 		    lhsvl = IS_NPC(rndm) ? 0 : PLAYERCLAN(rndm);
 		    rhsvl = atoi(val);
 		    return mprog_veval(lhsvl, opr, rhsvl);
 		  }
 	          else
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'clannum'",
                 mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "sex"))
     {
       if (isdigit(*val))
 	            ...
 	            rhsvl = search_block_flag(val, genders);
       switch (arg[1])  /* arg should be "$*" so just get the letter */
 	{
   ...
   if (!str_cmp(buf, "position"))
     {
       if (isdigit(*val))
 	            ...
 	            rhsvl = search_block_flag(val, position_types);
       switch (arg[1])  /* arg should be "$*" so just get the letter */
 	{
   ...
   if (!str_cmp(buf, "class"))
     {
       if (isdigit(*val))
 	            ...
 	            rhsvl = search_block_flag(val, pc_class_types);
       switch (arg[1])  /* arg should be "$*" so just get the letter */
 	{
 	...
 	case 'r': if (rndm)
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'class'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "goldamt"))
 	...
 	case 'r': if (rndm)
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'goldamt'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "objtype"))
     {
       if (isdigit(*val))
 	            ...
 	            rhsvl = search_block_flag(val, item_types);
       switch (arg[1])  /* arg should be "$*" so just get the letter */
 	{
 	...
 	case 'p': if (v_obj)
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'objtype'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "objval0"))
     ...
     rhsvl = atoi(val);
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'objval0'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "objval1"))
 	...
 	case 'p': if (v_obj)
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'objval1'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "objval2"))
 	...
 	case 'p': if (v_obj)
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'objval2'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "objval3"))
 	...
 	case 'p': if (v_obj) 
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'objval3'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "number"))
 	...
 	case 'p': if (v_obj)
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'number'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
   ...
   if (!str_cmp(buf, "name"))
 	...
 	case 'p': if (v_obj)
 		    ...
 		    return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'name'", mob_index[mob->nr].vnum); 
 	  return -1;
 	}
     }
     
    if (!str_cmp(buf, "leavedir"))
     {
       switch (arg[1])  /* arg should be "$*" so just get the letter */
 	{
 	case 'i': if (GET_LEAVE_DIR(mob) >= 0 && GET_LEAVE_DIR(mob) < NUM_OF_DIRS)
 	            return mprog_seval(dirs[(int) GET_LEAVE_DIR(mob)], opr, val);
 	          return -1;  
 	case 'n': if (actor)
 	            if (GET_LEAVE_DIR(actor) >= 0 && GET_LEAVE_DIR(actor) < NUM_OF_DIRS)
 	              return mprog_seval(dirs[(int) GET_LEAVE_DIR(actor)], opr, val);
 	          return -1;
 	case 't': if (vict)
 	            if (GET_LEAVE_DIR(vict) >= 0 && GET_LEAVE_DIR(vict) < NUM_OF_DIRS)
 	              return mprog_seval(dirs[(int) GET_LEAVE_DIR(vict)], opr, val);
 	          return -1;
 	case 'r': if (rndm)
 	            if (GET_LEAVE_DIR(rndm) >= 0 && GET_LEAVE_DIR(rndm) < NUM_OF_DIRS)
 	              return mprog_seval(dirs[(int) GET_LEAVE_DIR(rndm)], opr, val);
 	          return -1;
 	default:
 	  bug ("Mob: %d bad argument to 'name'", mob_index[mob->nr].vnum); 
 	  return -1;
    ...
    */
   bug ("Mob: %d unknown ifchck", mob_index[mob->nr].vnum); 
   return -1;
  ...
  *null = '\0';
  ...
  while(loopdone == FALSE) /*scan over any existing or statements */
  {
      ...
      if (*cmnd == '\0')
      {
 	 bug ("Mob: %d no commands after IF/OR", mob_index[mob->nr].vnum); 
 	 return null;
      }
  ...
  if (flag)
    ...
    return com_list; 
 	 ...
 	 cmnd++;
        if (*cmnd == '\0')
        {
            bug ("Mob: %d missing else or endif", mob_index[mob->nr].vnum); 
            return null;
        }
        morebuf = one_argument(cmnd, buf);
    }
  else /*false ifcheck, find else and do existing commands or quit at endif*/
    {
      ...
      if (*cmnd == '\0')
        { 
 	 bug ("Mob: %d missing endif", mob_index[mob->nr].vnum); 
 	 return null;
        }
      morebuf = one_argument(cmnd, buf);
  ...
  *t = '\0';
  switch (ch) {
      ...
      default:
          bug("Mob: %d bad $var", mob_index[mob->nr].vnum);
 	 break;
        }
  ...
  */
 char *mprog_driver (char *com_list, struct char_data *mob, struct char_data *actor,
 		   struct obj_data *obj, void *vo)
 {
 ...
 {
   MPROG_ACT_LIST *tmp;
   if (!(mob->mpact->pos = mprog_driver(mob->mpact->pos, mob, mob->mpact->ch, 
     mob->mpact->obj, mob->mpact->vo))) {
     tmp = mob->mpact;
     mob->mpact = mob->mpact->next;
     free(tmp);
     (mob->mpactnum)--;
   } 
 }
 			  ...
 			  struct obj_data *obj, void *vo, int type)
 {
   ...
   if (!comlist || !*comlist || mob->mpactnum >= MAX_PROG_QUEUE) return;
   CREATE(tmp, MPROG_ACT_LIST, 1);
   tmp->next = mob->mpact; 
   tmp->buf = comlist;
   tmp->pos = comlist;
   tmp->ch = actor;
 ...
 void mprog_wordlist_check(char *arg, struct char_data *mob, struct char_data *actor,
 			  struct obj_data *obj, void *vo, int type, bool enqueue)
 {
 	...
 	if ((list[0] == 'p') && (list[1] == ' '))
 	  {
 		...
 		{
 		  tmp = mprg->comlist;
 		  if (enqueue || (tmp =  mprog_driver(mprg->comlist, mob, actor, obj, vo)))
 		    mprog_new_program(tmp, mob, actor, obj, vo, type);
 		  break;
 	  ...
 	  { 
 		...
 		if ((start == dupl || *(start-1) == ' ')
 			...
 			|| *end == '\0'))
 		  {
 		    if ((tmp =  mprog_driver(mprg->comlist, mob, actor, obj, vo)))
 		      mprog_new_program(tmp, mob, actor, obj, vo, type);
 		    break;
 		  }
 		else
...
//   MPROG_ACT_LIST * tmp_act;
   ...
   if (IS_NPC(mob)
       ...
       && (mob != ch))
     {
       mprog_wordlist_check(buf, mob, ch, obj, vo, ACT_PROG, TRUE); 
     /*
 ...
 void mprog_bribe_trigger(struct char_data *mob, struct char_data *ch, int amount)
 {
  ...
  if (IS_NPC(mob)
 	...
 	if ((mprg->type & BRIBE_PROG)
 	  ...
 	  {
 	    if ((tmp =  mprog_driver(mprg->comlist, mob, ch, obj, NULL)))
 		      mprog_new_program(tmp, mob, ch, obj, NULL, BRIBE_PROG);
 	    break;
 	  }
     }
 ...
 void mprog_death_trigger(struct char_data *mob, struct char_data *killer)
 {
  ...
  if (IS_NPC(mob)
    ...
    {
      mprog_percent_check(mob, killer, NULL, NULL, DEATH_PROG, FALSE);
    }
 ...
 void mprog_entry_trigger(struct char_data *mob)
 {
  ...
  if (IS_NPC(mob)
    ...
    mprog_percent_check(mob, NULL, NULL, NULL, ENTRY_PROG, FALSE);
 ...
 void mprog_leave_trigger(struct char_data *ch)
 {
  ...
  for (vmob = world[ch->in_room].people; vmob != NULL; vmob = vmob->next_in_room)
    if (IS_NPC(vmob)
        ...
        && AWAKE(vmob)
        && (mob_index[vmob->nr].progtypes & LEAVE_PROG))
      mprog_percent_check(vmob, ch, NULL, NULL, LEAVE_PROG, FALSE);
  return;
 ...
 void mprog_fight_trigger(struct char_data *mob, struct char_data *ch)
 {
  ...
  if (IS_NPC(mob)
    ...
    mprog_percent_check(mob, ch, NULL, NULL, FIGHT_PROG, FALSE);
 ...
 void mprog_give_trigger(struct char_data *mob, struct char_data *ch, struct obj_data *obj)
 {
  ...
  if (IS_NPC(mob)
    ...
    for (mprg = mob_index[mob->nr].mobprogs; mprg != NULL; mprg = mprg->next)
 	 ...
 	 {
 	   if ((tmp =  mprog_driver(mprg->comlist, mob, ch, obj, NULL)))
 		      mprog_new_program(tmp, mob, ch, obj, NULL, GIVE_PROG);
 ...
 void mprog_greet_trigger(struct char_data *ch)
 {
  ...
  for (vmob = world[ch->in_room].people; vmob != NULL; vmob = vmob->next_in_room)
    if (IS_NPC(vmob)
      ...
      mprog_percent_check(vmob, ch, NULL, NULL, GREET_PROG, FALSE);
    else
      if (IS_NPC(vmob)
 	 && (vmob->char_specials.fighting == NULL)
 	 && AWAKE(vmob)
 	 && (mob_index[vmob->nr].progtypes & ALL_GREET_PROG))
        mprog_percent_check(vmob,ch,NULL,NULL,ALL_GREET_PROG, FALSE);
  ...
  if (IS_NPC(mob)
    ...
    for (mprg = mob_index[mob->nr].mobprogs; mprg != NULL; mprg = mprg->next)
 	 ...
 	 && ((100*mob->points.hit / mob->points.max_hit) < atoi(mprg->arglist)))
        {
         if ((tmp =  mprog_driver(mprg->comlist, mob, ch, NULL, NULL)))
 		      mprog_new_program(tmp, mob, ch, NULL, NULL, HITPRCNT_PROG);
 	 break;
        }
 ...
 void mprog_random_trigger(struct char_data *mob)
 {
   if (mob_index[mob->nr].progtypes & RAND_PROG)
     mprog_percent_check(mob,NULL,NULL,NULL,RAND_PROG,FALSE);
 ...
 void mprog_speech_trigger(char *txt, struct char_data *mob)
 {
   ...
   for (vmob = world[mob->in_room].people; vmob != NULL; vmob = vmob->next_in_room)
     if ((mob != vmob) && IS_NPC(vmob) && (mob_index[vmob->nr].progtypes & SPEECH_PROG))
       mprog_wordlist_check(txt, vmob, mob, NULL, NULL, SPEECH_PROG, FALSE);
...
void mprog_reset_trigger(struct char_data *mob)
 {
  ...
  if (IS_NPC(mob)
      && (mob_index[mob->nr].progtypes & RESET_PROG))
    {
      mprog_percent_check(mob, NULL, NULL, NULL, RESET_PROG, TRUE);
    }
...
void mprog_time_trigger(struct char_data *mob)
 {
  ...
  if (IS_NPC(mob) && (mob_index[mob->nr].progtypes & TIME_PROG)) {
    for (mpact = mob->mpact; mpact != NULL; mpact = mpact->next)
      if (mpact->progtype == TIME_PROG) return;
    for (mprg = mob_index[mob->nr].mobprogs; mprg != NULL; mprg = mprg->next)
      if ((mprg->type & TIME_PROG) && time_info.hours == atoi(mprg->arglist)) {
        if ((tmp =  mprog_driver(mprg->comlist, mob, NULL, NULL, NULL)))
 		      mprog_new_program(tmp, mob, NULL, NULL, NULL, TIME_PROG);
 	break;
      }
  }
  return; 
